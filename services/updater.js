...
updater.alterSourceData = async (osc_message) => {
    ...
    if (osc_message.args.length > 0) {
        argType = osc_message.args[0].type;
        argValue = osc_message.args[0].value;
    ...
    let addWorking = "";
        ...
        // LOG MESSAGE IN OSC LOG
    let updateMessage = "CONSOLE : <strong style='color:goldenrod;'>" + address +  "</strong> : " + argValue + "  (<i>" + argType + "</i>)<br>";
    // let logText = $("#ascConsoleLog").html();
    //let res = updateMessage + logText;
    //$("#ascConsoleLog").html(res);
    ...
    if (address == '/eos/out/ping') {
      ...
      reset();
      requestFaders(1, appState.faderPage.page, 10);
      requestDS();
      ...
      if (value.includes('[')) {
        ...
        if(typeof argArr[0] !== "undefined")

          {
            ...
            // console.log("Active Chan :" + appState.activeChan);
...
*/  
      ...
      appState.faders[bankNum-1][faderNum-1].enabled = true;
      ...
      for (let i = 0; i < 12; i++) {
        ...
        //console.log(appState.encoders[i]);

        if (appState.encoders[i].centerText == centerText && centerText != "") {

          if (appState.encoders[i].mode == 'superfine') {
            appState.encoders[i].address = "/eos/active/wheel/fine/"+wheelNumber;
          } else {
            appState.encoders[i].address = "/eos/active/wheel/"+wheelNumber;
          }

          appState.encoders[i].wheel = wheelNumber;
          enableEncoder(i);
          encNum = i+1;
    ...
    let addArr = address.split('/');
        ...
        appState.ds[dsModule - 1].label = label;
        appState.ds[dsModule - 1].page = pageNum;

        // $('[data-asc-id="ds'+ dsModule +'-page').text("Page " + appState.ds[dsModule - 1].page);

        // This is the last thing to happen with DS
        // We can Store the App State

        storeFile(appState, 'appState');
    }
